...
analysis are adopted in an \textit{ad hoc} manner. The experimentally verified binding
...
sites in the Bacteria domain (Chapter~\ref{chap:collectf}). Having binding site
...
target species, their use of \textit{ad hoc} methods to combine findings in each target
...
the evolution of regulation of individual gene components.
...
evaluation of transfer methods, we have developed CGB (Comparative Genomics of
Bacteria), a comparative genomics
...
probability of its regulation, ranging from green (high posterior probability
of regulation) to red
(low posterior probability of regulation). The absence of an ortholog in a species for a particular
...
Bacterial genes are organized into clusters of coregulated genes, called
operons~\citep{jacob1960operon}.
...
\begin{figure}
  ...
  operon prediction.]{Determining the species-specific intergenic distance
    threshold for operon prediction. (a) Histograms of intergenic distances of
    adjacent genes that are on the same strand. They demonstrate that pairs of
    genes within the same operon (WO) are closer than pairs in different
    operons (TUB). Also, the intergenic distance distribution is different for
    each species. (b) Percent operon prediction accuracy as a function of
    intergenic distance threshold (bp). The mean intergenic distances and the
    prediction accuracy of using them as thresholds are shown with larger
    markers. Since operons are predicted to search for binding sites, each
    operon is evaluated as a structure of regulation. Hence, accuracy is
    defined as the ratio of genes that are assigned to their true promoter.}
...
to 50 bp downstream with respect to transcriptional start site by
default~\citep{gralla1996organization}].  For each sequence position, we
combine the scores from both strands using the softmax function:
...
RegPredict~\citep{bhargava2010xfitom, novichkov2010regpredict}. Eleven species from
...
used. The reference LexA binding site collections were obtained from CollecTF,
compiled from experimentally validated sites for \textit{B. subtilis},
